...
function jajanBoba(uang: number, callback: (uang: number) => void) {
  ...
  if (uang >= hargaBoba) {
    setTimeout(() => {
      ...
      console.log(
        `kamu jajan boba dengan harga Rp. ${hargaBoba} sisa uang kamu Rp. ${sisaUang}`
      );
      callback(sisaUang); // Memanggil callback dengan sisa uang untuk membeli seblak
    }, 5000); // Estimated time to get boba: 5 seconds
  } else {
    console.log(
      `Maaf uang kamu belum cukup untuk membeli boba. Sisa uang kamu sebesar Rp. ${uang}`
    );
  }
}
...
function jajanSeblak(uang: number) {
  ...
  if (uang >= hargaSeblak) {
    setTimeout(() => {
      ...
      console.log(
        `kamu jajan seblak dengan harga Rp. ${hargaSeblak} sisa uang kamu Rp. ${sisaUang}`
      );
    }, 9000); // Estimated time to get seblak: 9 seconds
  } else {
    console.log(
      `Maaf uang kamu belum cukup untuk membeli seblak. Sisa uang kamu sebesar Rp. ${uang}`
    );
  }
}
